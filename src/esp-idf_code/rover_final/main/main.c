...
void run_trashe() {
	...
	// While we have not finished a full run, continue tracking the line and detecting obstacles
	while(!done)
	{
		//digitalWrite(ULTRA_DIRECTION, 1);
		objectDetected = digitalRead(ULTRA_DETECTION);
		// towards_pickup indicates our direction of movement,
		// true => forwards on the line to the pickup location (thus direction pin becomes 1)
		// false => backwards on the line to home (thus direction pin becomes 0)
		// The direction pin gets controlled in the trackLine function, no need to explicitly do it

		while(at_pickup)
		{
			// send at pickup message
			stop();
			// wait until load is picked up when at pickup location or until timer runs out
			int startTime = millis();
			while (millis() - startTime < load_removal_wait_time) {
				loadDetected = digitalRead(LOAD_DETECTION);
				if (!loadDetected) {
		...
		if(objectDetected)
		{
			...
			{
				objectDetected = digitalRead(ULTRA_DETECTION);
				// Only try to change directions after a predetermined amount of time when we are moving towards the pickup location
				if((millis() - startTime >= load_removal_wait_time) && towards_pickup)
				{
					towards_pickup = false;
					break;
				}
		...
		switch(trackLine(ADC1_CHANNEL_3, ADC1_CHANNEL_6, true, &photoValStruct, towards_pickup))
		{
			case 1: // Left side triggered
				...
				stop();
				vTaskDelay(200/portTICK_RATE_MS);
				if (towards_pickup) {rotate_right(motorSpeed);}
				else {rotate_left(motorSpeed);}
				vTaskDelay(500 / portTICK_RATE_MS);
				break;
			...
			case 2: // Right side triggered
				...
				stop();
				vTaskDelay(200 / portTICK_RATE_MS);
				if (towards_pickup) {rotate_left(motorSpeed);}
				else {rotate_right(motorSpeed);}
				vTaskDelay(500 / portTICK_RATE_MS);
				break;
			case 3: // At Pickup Location
				stop();
...
void app_main(void)
{
	...
	while(1)
	{
		...
		/*if (!(boo++)) {send_message("ready");}
		else {send_message("ready");}*/
		//send_message("ready");
